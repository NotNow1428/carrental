...
{ 
    $userName = htmlentities($result->FullName);
    $userEmail = htmlentities($result->EmailId);
    $userPhone = htmlentities($result->ContactNo);
    ?>
...
foreach($results as $result)
{  
    $vehicleName = htmlentities($result->VehiclesTitle) . ', ' . htmlentities($result->BrandName);
    $bookingNo = htmlentities($result->BookingNumber);
    $totalDays = htmlentities($result->totaldays);
    $pricePerDay = htmlentities($result->PricePerDay);
    $grandTotal = $totalDays * $pricePerDay;
    ?>

                                    <li>
                                        <h4 style="color:red">Booking No
                                            #<?php echo $bookingNo;?></h4>
                ...
                { ?>
                                    ...
                                    <?php if($result->Status==0) {          // Check if not confirmed ?>
                                    <div style="text-align: right; margin-top: 10px;">
                                        <a href="KhaltiPayment/checkout.php?bookingNo=<?php echo $bookingNo;?>&vehicleName=<?php echo urlencode($vehicleName);?>&amount=<?php echo $grandTotal;?>&name=<?php echo urlencode($userName);?>&email=<?php echo urlencode($userEmail);?>&phone=<?php echo urlencode($userPhone);?>"><button class="btn" style="background-color: green; color: white;border-radius:20px;">Payment</button></a>
                                    </div>
                                    <?php } ?>
                                    <hr />
                                    <?php }}  else { ?>
